      ...
      getTimeSlots (callback) {
        ...
        if (appointment.serviceId) {
          ...
          this.$http.get(`${this.$root.getAjaxUrl}/slots`, {
            ...
            .then(response => {
              callback(this.selectedTimeZone === 'UTC' && this.$root.settings.general.showClientTimeZone ? this.getConvertedTimeSlots(response.data.data.slots) : response.data.data.slots)
              this.dialogLoading = false
