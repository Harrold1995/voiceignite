<template>
  <div class="am-wrap">
    <transition name="fade">
      <div v-if="trafftBannerVisible" class="am-trafft-banner">
        <div class="am-trafft-banner__content">
          <div class="am-trafft-banner__content-main">
            <div class="am-trafft-banner__content-logo">
              <img :src="$root.getUrl + 'public/img/am-trafft-logo.svg'" alt="Trafft New">
            </div>
            <div class="am-trafft-banner__content-text">
              <h1>
                <img :src="$root.getUrl + 'public/img/am-trafft-logo.svg'" alt="Trafft New">
                <span>Trafft</span>
                <span class="am-trafft-banner__badge">{{ $root.labels.new }}</span>
              </h1>
              <p>
                {{ $root.labels.trafft_text1 }}
              </p>
              <p>
                {{ $root.labels.trafft_text2 }}
              </p>
            </div>
          </div>
          <div class="am-trafft-banner__content-coupon">
            {{ $root.labels.coupon_text }}
          </div>
        </div>

         <div class="am-trafft-banner__buttons">
           <p @click="removeBanner">{{ $root.labels.not_show }}</p>
           <a class="am-trafft-banner__button" href="https://trafft.com" target="_blank">{{ $root.labels.learn_more }}</a>
         </div>
      </div>
    </transition>
...
<script>
  ...
  export default {
    ...
    created () {
      ...
      if (localStorage.getItem('trafftBanner') !== null) {
        this.trafftBannerVisible = JSON.parse(localStorage.getItem('trafftBanner'))
      }
    },

    methods: {
      removeBanner () {
        localStorage.setItem('trafftBanner', false)
        this.trafftBannerVisible = false
      },
...
.am-trafft-banner {
  ...
  margin-bottom: 20px;
  border-radius: 8px;
  background: linear-gradient(88.62deg, #2A87F5 -15.15%, rgba(42, 135, 245, 0) 57.75%), #00124F;
