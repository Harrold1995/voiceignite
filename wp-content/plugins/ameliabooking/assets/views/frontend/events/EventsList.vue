                        ...
                        <el-row>
                          ...
                          <el-col :sm="12" :lg="12" class="am-ticket-number-of-people">
                            ...
                            <el-input-number
                                ...
                                :min="0"
                                :max="evt.bringingAnyone ? ticket.spots - ticket.sold : 1"
                                type="number"
                                @blur="() => {
    ...
    methods: {
      setTicketsData (event) {
        ...
        event.customTickets.forEach((ticket, index) => {
          event.bookingToEventTickets.push({
            id: Math.random(),
            customerBookingId: null,
            eventTicketId: ticket.id,
            persons: !enabledFirst && ticket.enabled ? 1 : 0,
            price: ticket.dateRangePrice ? ticket.dateRangePrice : ticket.price
          })

          if (!enabledFirst && ticket.enabled && ticket.spots - ticket.sold > 0) {
            enabledFirst = true
          }
        })
        return event
        ...
        this.$http.get(`${this.$root.getAjaxUrl}/events`, {
          ...
          .then(response => {
            ...
            response.data.data.events.forEach(function (event) {
              ...
              if (event.customTickets.length && event.customPricing) {
                event = $this.setTicketsData(event)
              }
