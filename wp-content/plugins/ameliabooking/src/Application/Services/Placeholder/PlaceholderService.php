...
abstract class PlaceholderService implements PlaceholderServiceInterface
{
    ...
    protected function getBookingData($appointment, $type, $bookingKey = null, $token = null, $depositEnabled = null)
    {
        ...
        if (!empty($appointment['deposit']) || $depositEnabled) {
            if ($bookingKey !== null) {
                $deposit = !empty($appointment['bookings'][$bookingKey]['payments'][0]) ?
                    $appointment['bookings'][$bookingKey]['payments'][0] : null;

                $depositAmount = $deposit ? $deposit['amount'] : 0;
            } else {
                foreach ($appointment['bookings'] as $booking) {
                    if (!empty($booking['isLastBooking'])) {
                        $deposit = !empty($booking['payments'][0]) ?
        ...
        if ($appointment['type'] !== 'event') {
            $result["{$appointment['type']}_price"] = $helperService->getFormattedPrice($appointmentPrice);
        }
